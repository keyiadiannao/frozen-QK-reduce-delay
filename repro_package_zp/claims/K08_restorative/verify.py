"""K08 verify.py — reconcile package run vs archived repro pkl.

Bit-exact comparison of claims/K08_restorative/results.pkl against
repro/r94_perturb_recovery_results.pkl, per key (S0, F1, ...):
control (t/va_plain/cross95/cross90/strat/Rpre), codes_T0 (float16
bitwise), branches P50/P200 (t/va/cross/strat/Rctrl/Rpre/pulse_end/
va_at_end/g3_fail).
"""
import os
import pickle
...
def cmp_codes(tag, a, b):
    ...
    if a.shape != b.shape or a.dtype != b.dtype or \
        ...
        MISMATCH.append(f'{tag}: differs (shape={a.shape} vs '
                        f'{b.shape})')


def main():
    arch_path = os.path.join(REPRO_DIR,
                             'r94_perturb_recovery_results.pkl')
    ...
    if sorted(mine) != sorted(ref):
        print(f'[K08 verify] keys differ: {sorted(mine)} vs '
              f'{sorted(ref)}')
        sys.exit(1)
    for key in ref:
        r = ref[key]
        ...
        for br in r['branches']:
            rb = r['branches'][br]
            mb = m['branches'][br]
            cmp_list(f'{key}.{br}.t', mb['t'], rb['t'])
            cmp_list(f'{key}.{br}.va_plain', mb['va_plain'],
                     rb['va_plain'])
            for k in ('cross95', 'cross90', 'pulse_end', 'va_at_end',
                      'g3_fail'):
                cmp_scalar(f'{key}.{br}.{k}', mb[k], rb[k])
            for t in rb['strat']:
                for kk in rb['strat'][t]:
                    cmp_scalar(f'{key}.{br}.strat[{t}].{kk}',
                               mb['strat'][t][kk],
                               rb['strat'][t][kk])
            cmp_dict_of_scalars(f'{key}.{br}.Rctrl',
                                mb['Rctrl'], rb['Rctrl'])
            cmp_dict_of_scalars(f'{key}.{br}.Rpre',
                                mb['Rpre'], rb['Rpre'])
    if MISMATCH:
        print(f'[K08 verify] FAIL ({len(MISMATCH)} mismatches)')
        for x in MISMATCH[:10]:
            print('  ' + x)
        sys.exit(1)
    print('[K08 verify] PASS (bit-exact vs archived pkl: control '
